...
> **weight tying（权重绑定）**：`transformer.wte.weight = lm_head.weight`。输入嵌入和输出层共享同一份权重矩阵——按 GPT-2 默认尺寸（50304×768）省下 38,633,472 个参数（约 3900 万，不是 3000 万）；本仓库红楼梦配置（4339×384）省下 1,666,176 个。效果还更好。
